...
def runCocotbTestsRegisterFile(pytestconfig):
    ...
    verilog_sources = [ PROJECT_PATH / "DatapathSingleCycle.v" ]
...
def test_processor(pytestconfig):
    """run processor tests"""
    ...
    runr.test(
        ...
        # testcase=pytestconfig.option.tests, # filter tests via the `--tests` command-line flag
        testcase=None, # <--- Change this to None to run ALL tests
    )
    pass
...
async def testLuiAddi(dut):
    "Run two inst. to check PC incrementing"
...
async def testEcall(dut):
    "ecall inst. causes processor to halt"
    riscv_binary_utils.asm(dut, '''
        lui x1,0x12345
        ecall
        lui x1,0xABCDE''')
